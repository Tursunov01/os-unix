...
#define VIRTUAL_FC_MAGIC_NUM 0x13131313
#define SLAB_NAME "virtual_fc_cache"
...
static struct virtual_fc_inode
{
    int i_mode;
    unsigned long i_ino;
} virtual_fc_inode;

static struct inode *virtual_fc_make_inode(struct super_block *sb, int mode)
{
    ...
    if (ret)
    {
        ...
        ret->i_private = &virtual_fc_inode;
    }

    printk(KERN_INFO "[vfs] Создана struct inode\n");
    return ret;
}

// вывод строки перед уничтожением суперблока
static void virtual_fc_put_super(struct super_block *sb)
{
    printk(KERN_INFO "[vfs] Вызвана функция уничтожения суперблока\n");
}

// Операции структуры суперблок
static struct super_operations const virtual_fc_sup_ops = {
        .put_super = virtual_fc_put_super,
        // заглушки из libfs
...
static int virtual_fc_fill_sb(struct super_block *sb, void *data, int silent)
{
///////////////////// инициализировать поля структуры суперблока /////////////////////////
    ...
    //или прочие данные
    sb->s_magic = VIRTUAL_FC_MAGIC_NUM;

    //операции суперблока
    sb->s_op = &virtual_fc_sup_ops;
...
///////////////////// построить корневой каталог ФС/////////////////////////
    ...
    struct inode *root_inode = virtual_fc_make_inode(sb, S_IFDIR | 0755); //7 - все разрешено 5 - чтение и выполнение  владелец-член группы-другие
    if (!root_inode)
    {
        printk(KERN_ERR "[vfs] Ошибка virtual_fc_make_inode\n");
        return -ENOMEM;
    }
    ...
    if (!sb->s_root)
    {
        printk(KERN_ERR "[vfs] Ошибка d_make_root\n");
        iput(root_inode);  
        return -ENOMEM;
    }
    else
        printk(KERN_INFO "[vfs] Создан корневой каталог виртуальной файловой системы\n");

    printk(KERN_INFO "[vfs] Суперблок проинициализирован");
    return 0;
}

// вызывается при монтировании ФС:
// должна вернуть структуру, описывающую корневой каталог ФС
static struct dentry *virtual_fc_mount(struct file_system_type *type, int flags, const char *dev, void *data)
{
    // создается виртуальная ФС, несвязанная с устройстовом
    // virtual_fc_fill_sb - указатель на функцию, которая будет вызвана из mount_nodev для создания суперблока
    //nodev - Не различает файловые системы символьно-специальных и блочно-специальных устройств.
    struct dentry *const root_dentry = mount_nodev(type, flags, data, virtual_fc_fill_sb);

    if (IS_ERR(root_dentry))
        printk(KERN_ERR "[vfs] Ошибка  mount_nodev\n");
    else
        printk(KERN_INFO "[vfs] Виртуальная файловая система была монтирована\n");
...
static struct file_system_type virtual_fc_type = {
...
static int __init virtual_fc_init(void)
{
    // Регистрация файловой ситемы
    int ret = register_filesystem(&virtual_fc_type);
    if (ret != 0)
    {
        printk(KERN_ERR "[vfs] Ошибка register_filesystem\n");
        return ret;
    }
    ...
    if (!cache_mem_area)
    {
        printk(KERN_ERR "[vfs] Ошибка cache_mem_area\n");
    ...
    if (!cache)
    {
        printk(KERN_ERR "[vfs] Ошибка kmem_cache_create\n");
    ...
    {
        printk(KERN_ERR "[vfs] Ошибка kmem_cache_alloc\n");
    ...
    printk(KERN_INFO "[vfs] Модуль виртуальной файловой системы загружен");
    return 0;
}

static void __exit virtual_fc_exit(void)
{
    kmem_cache_free(cache, *cache_mem_area);
    kmem_cache_destroy(cache);
    kfree(cache_mem_area);

    if (unregister_filesystem(&virtual_fc_type) != 0)
    {
        printk(KERN_ERR "[vfs] Ошибка unregister_filesystem\n");
    }

    printk(KERN_INFO "[vfs] Модуль виртуальной файловой системы выгружен \n");
}

module_init(virtual_fc_init);
module_exit(virtual_fc_exit);
